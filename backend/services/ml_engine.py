...
def run_forecasting(df: pd.DataFrame, periods: int = 6) -> list | dict:
    ...
    if not date_col:
        return {"error": "Datetime kolon bulunamadı. Forecasting için tarih kolonu gerekli."}

    num_cols = df.select_dtypes(include=[np.number]).columns.tolist()
    if not num_cols:
        return {"error": "Numerik kolon bulunamadı."}
...
def run_clustering(df: pd.DataFrame, selected_cols: list = None, max_k: int = 5) -> dict:
    num_cols_available = df.select_dtypes(include=[np.number]).columns.tolist()

    if len(num_cols_available) < 2:
        return {"error": "Clustering için en az 2 numerik kolon gerekli."}

    if not selected_cols:
        variances = df[num_cols_available].var().sort_values(ascending=False)
    ...
    scatter_data = df_result[[selected_cols[0], selected_cols[1], "cluster"]].copy()
    scatter_data["cluster"] = scatter_data["cluster"].astype(str)

    return sanitize_for_json({
        "id": "clustering-result",
        "type": "clustering",
        "title": f"{detect_cluster_domain(df)} ({optimal_k} Clusters)",
        "optimal_k": optimal_k,
        ...
        "scatter_col_x": selected_cols[0],
        "scatter_col_y": selected_cols[1],
        "insight": (
            f"Optimal k={optimal_k} (elbow method). {len(df_result)} records segmented across "
            f"{len(selected_cols)} features. Cluster quality should be read with silhouette and overlap warnings."
        ),
